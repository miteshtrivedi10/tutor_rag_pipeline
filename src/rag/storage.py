...
class ElementRelation(BaseModel):
    ...
    relation_type: str = Field(..., description="Type of relationship (describes, illustrates, etc.)")
    ...
    spatial_proximity: Optional[float] = Field(None, ge=0, le=1, description="Normalized spatial distance")

    @validator('relation_type')
    def validate_relation_type(cls, v):
        valid_types = ["describes", "illustrates", "supports", "contains", "references", 
                       "explains", "demonstrates", "complements", "contrasts"]
    ...
    def validate_direction(cls, v):
        valid_directions = ["text_to_visual", "visual_to_text", "element_to_element", "bidirectional"]
...
class MilvusStorage:
    ...
    def __init__(self, uri: str, token: str,
                 collection_name: str = "rag_embeddings_enhanced", auto_create: bool = True,
        ...
        try:
            ...
            if self.client.has_collection(self.collection_name):
                ...
                try:
                    ...
                    try:
                        ...
                        logger.info(f"Collection {self.collection_name} is empty or not ready for queries")
                    return
                except Exception as load_error:
                    logger.error(f"Failed to load collection even after index creation: {load_error}")
            ...
            schema = CollectionSchema(
                fields=[
                    ...
                    FieldSchema(name="text_content", dtype=DataType.VARCHAR, max_length=self.MAX_TEXT_LENGTH),
                    ...
                    FieldSchema(name="processing_timestamp", dtype=DataType.VARCHAR, max_length=50),
                ],
    ...
    def _create_indexes(self):
        ...
        try:
            ...
            logger.info(f"High recall vector index created for collection {self.collection_name}")
        except Exception as e:
            # Check if the error is about index already existing
            error_msg = str(e).lower()
            if "at most one distinct index is allowed per field" in error_msg or "index already exists" in error_msg or "index built" in error_msg:
                logger.info(f"Index already exists for {self.collection_name}")
                return
            else:
                raise FileProcessingError(f"Failed to create required index: {e}")

    def insert_single(self, embedding: List[float], text_content: str, content_type: str,
                     source_file: str, page_id: str, metadata: Optional[Dict] = None,
                     processing_timestamp: Optional[str] = None) -> str:
        ...
        try:
            ...
            logger.debug(f"Inserted document {doc_id} into collection {self.collection_name}")
            return doc_id
    ...
    def insert_batch(self, embeddings_data: List[Dict]) -> List[str]:
        ...
        try:
            ...
            if entities:
                ...
                logger.debug(f"Inserted final batch of {len(entities)} documents")
            
            logger.info(f"Successfully inserted {len(doc_ids)} documents into {self.collection_name}")
            return doc_ids
    ...
    def search_similar_content(self, query_embedding: List[float], top_k: int = 5,
        ...
        try:
            ...
            expr = None
            if content_types:
                content_expr = " OR ".join([f"content_type == '{ct}'" for ct in content_types])
                expr = content_expr
            if source_filter:
            ...
            results = self.client.search(
                ...
                limit=top_k,
                filter=expr,
                output_fields=["id", "text_content", "content_type", "source_file", "page_id", "metadata", "processing_timestamp"]
            )
            ...
            for result in results[0]:
                doc = {
                    ...
                    "processing_timestamp": result.entity.get("processing_timestamp", ""),
...
class MockMilvusStorage:
    ...
    def search_similar_content(self, query_embedding: List[float], top_k: int = 5, 
        ...
        if content_types:
            filtered_docs = [doc for doc in filtered_docs if doc["content_type"] in content_types]
        if source_filter:
            filtered_docs = [doc for doc in filtered_docs if doc["source_file"] == source_filter]
