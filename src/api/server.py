...
async def search_similar_content(
    ...
    try:
        similar_items = rag_processor.search_similar_content(query, top_k)
        return {
            "query": query,
...
async def search_similar_content_with_reranking(
    ...
    try:
        # Use the basic search method since reranking method doesn't exist
        similar_items = rag_processor.search_similar_content(query, final_top_k)
        return {
            "query": query,
            "results": similar_items,
            "count": len(similar_items),
...
async def search_similar_content_by_type(
    ...
    try:
        # Use the basic search method with content type filtering
        similar_items = rag_processor.search_similar_content(
            query, top_k, content_types=[content_type]
        )
        return {
            "query": query,
...
async def search_similar_content_by_chapter(
    ...
    try:
        # Use the basic search method - chapter filtering would need to be implemented
        similar_items = rag_processor.search_similar_content(query, top_k)
        return {
            "query": query,
            "chapter": chapter,
...
async def search_similar_content_by_source(
    ...
    try:
        # Use the basic search method with source file filtering
        similar_items = rag_processor.search_similar_content(
            query, top_k, source_filter=source_file
        )
        return {
            "query": query,
